...
from .config import Config, _focus_label, roadmap_areas, roadmap_focus
        ...
        if name == "roadmap":
            if sv.roadmap_backpressure:
                t.add_row(
                    ...
                    kind_cell,
                    "⛔",
                    f"backpressure: {sv.n_mine_open}/{MAX_OPEN_PRS} open  (focus: {focus})",
                    style=style,
                )
            else:
...
def launch_cmd(only: str | None, model_dial: str, host: bool, loop: bool, focus: str | None = None) -> list[str]:
...
def _dashboard_app(cfg, loader=None):
    ...
    class FocusPicker(ModalScreen):
        ...
        def compose(self) -> ComposeResult:
            ...
            yield Vertical(
                Label("roadmap focus", classes="title"),
                ol,
    ...
    class Dashboard(App):
                    ...
                    if name in self.expanded:
                        if not self.areas:
                            ...
                            for a in ["(all areas)"] + self.areas:
                                active = a == sv.roadmap_focus or (
                                    a == "(all areas)" and sv.roadmap_focus in ("", "any")
                                )
                                t.add_row(
                                    Text(""),
                                    Text("    └ " + a, style="dim"),
        ...
        def _render_status(self) -> None:
            ...
            s.append(sandbox)
            s.append("   roadmap focus=", style="bold")
            s.append(_focus_label(self.sv))
