...
## Choose a target
- **First, avoid duplicating open work.** List the PRs already in flight and read their titles and descriptions: `gh pr list --repo FormalFrontier/TauCeti --state open --limit 100 --json number,title,headRefName,body`. Do NOT pick a target that an open PR already covers or substantially overlaps (e.g. the same definition, the same roadmap target, or a near-identical API). If everything lowest-hanging is already taken, pick a genuinely different area or a clean prerequisite none of the open PRs supply. When in doubt that your idea is distinct, choose something else.
